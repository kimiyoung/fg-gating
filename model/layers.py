...
class FixEmbeddingLayer(lasagne.layers.Layer):
    def __init__(self, incoming, input_size, output_size,
                 W=lasagne.init.Constant(), **kwargs):
        super(FixEmbeddingLayer, self).__init__(incoming, **kwargs)

        self.input_size = input_size
        self.output_size = output_size

        self.W = self.add_param(W, (input_size, output_size), name="W", trainable = False, regularizable = False)
...
class GateDymNoWordLayer(lasagne.layers.MergeLayer):

    def __init__(self, incomings, W = lasagne.init.GlorotUniform(), b = lasagne.init.Constant(), **kwargs):
        super(GateDymNoWordLayer, self).__init__(incomings, **kwargs)
        units = self.input_shapes[1][2]
        feat_size = self.input_shapes[0][2]
        self.time_len = self.input_shapes[1][1]
        self.W = self.add_param(W, (feat_size, units), name = "W")
        self.b = self.add_param(b, (units, ), name = "b")

    def get_output_shape_for(self, input_shapes):
        return input_shapes[1]

    def get_output_for(self, inputs, **kwargs):
        # x_feat: (batch, time, feat); x_char, x_word: (batch, time, units)
        x_feat, x_char, x_word = inputs[0], inputs[1], inputs[2]
        W_gate = T.nnet.sigmoid(T.dot(x_feat, self.W) + self.b.dimshuffle('x', 'x', 0)) # (batch, time, units)
        return W_gate * x_char + (1 - W_gate) * x_word

class GateDymLayer(lasagne.layers.MergeLayer):

    def __init__(self, incomings, W = lasagne.init.GlorotUniform(), b = lasagne.init.Constant(), **kwargs):
        super(GateDymLayer, self).__init__(incomings, **kwargs)
        units = self.input_shapes[1][2]
        feat_size = self.input_shapes[0][2]
        self.time_len = self.input_shapes[1][1]
        self.W = self.add_param(W, (feat_size + units, units), name = "W")
        self.b = self.add_param(b, (units, ), name = "b")
    ...
    def get_output_for(self, inputs, **kwargs):
        # x_feat: (batch, time, feat); x_char, x_word: (batch, time, units)
        x_feat, x_char, x_word = inputs[0], inputs[1], inputs[2]
        x_feat = T.concatenate([x_feat, x_word], axis = 2) # (batch, time, feat)
        W_gate = T.nnet.sigmoid(T.dot(x_feat, self.W) + self.b.dimshuffle('x', 'x', 0)) # (batch, time, units)
        return W_gate * x_char + (1 - W_gate) * x_word

class ScalarDymLayer(lasagne.layers.MergeLayer):

    def __init__(self, incomings, W = lasagne.init.GlorotUniform(), b= lasagne.init.Constant(), **kwargs):
        super(ScalarDymLayer, self).__init__(incomings, **kwargs)
        units = self.input_shapes[1][2]
        feat_size = self.input_shapes[0][2]
        self.time_len = self.input_shapes[1][1]
        self.W = self.add_param(W, (feat_size + units, 1), name = "W")
    ...
    def get_output_for(self, inputs, **kwargs):
        x_feat, x_char, x_word = inputs[0], inputs[1], inputs[2]
        x_feat = T.concatenate([x_feat, x_word], axis = 2)
        W_gate = T.nnet.sigmoid(T.dot(x_feat, self.W) + self.b.dimshuffle('x', 'x', 0)).squeeze().dimshuffle(0, 1, 'x') # (batch, time, 1)
        return W_gate * x_char + (1 - W_gate) * x_word

class MatrixAttentionLayer(lasagne.layers.MergeLayer):
    def __init__(self, incomings, **kwargs):
        # incomings: [l_p_gru, l_q_gru, l_q_mask_inputs, l_match_feat]
        super(MatrixAttentionLayer, self).__init__(incomings, **kwargs)
        self.units = self.input_shapes[0][2]
        self.v1 = self.add_param(lasagne.init.GlorotUniform(), (self.units, 1), name = "v1")
        self.v2 = self.add_param(lasagne.init.GlorotUniform(), (self.units, 1), name = "v2")

    def get_output_shape_for(self, input_shapes):
        return (input_shapes[0][0], input_shapes[0][1], self.units)
...
class AttentionSumLayer(L.MergeLayer):
    ...
    def __init__(self, incomings, aggregator, pointer, mask_input=None, **kwargs): 
        """aggregator: cand_var, pointer: cloze_var, mask_input: candmask_var"""
